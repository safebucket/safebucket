...
This guide provides comprehensive instructions for deploying SafeBucket on AWS using Terraform. The infrastructure
includes:
...
The SafeBucket application requires specific IAM permissions to function correctly. **Missing any of these will cause
deployment failures.**
...
- **Lifecycle Permissions**: The application calls `EnsureTrashLifecyclePolicy()` at startup (internal/core/storage.go:
  25). Without these permissions, the application will exit with a fatal error.
- **Tagging Permissions**: Used for file metadata operations and trash marking (internal/storage/aws.go:195-261).
  Operations will fail with Access Denied errors.
