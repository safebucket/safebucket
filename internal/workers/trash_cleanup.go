...
func (w *TrashCleanupWorker) cleanupExpiredFiles(ctx context.Context) int {
	...
	// Adapt our publisher to Watermill's interface for TrashExpiration
	adapter := &watermillPublisherAdapter{publisher: w.Publisher}

	for _, file := range files {
		// Trigger TrashExpiration event with the marker path format
		// The event handler will delete the marker, file, and DB record
...
func (w *TrashCleanupWorker) cleanupExpiredFolders(ctx context.Context) int {
	...
	// Also handle nested folders whose parents are already deleted
